...
Hybrid concurrency strategy combining true reader-writer locks (@esfx/async-readerwriterlock) with optimistic concurrency control (mtime-based) [🏗️🔒💡]
...
Optimistic concurrency control pattern: (1) Capture mtime BEFORE lock, (2) Acquire lock, (3) Check mtime AFTER lock, (4) Throw if changed [⚠️🤯]
...
Current limitation: Locks are in-memory only (not cross-process filesystem locks) [⚠️💀]
...
CRITICAL LIMITATION OF MTIME **SOLVED** (Session 76621b6d design, be292b0b implementation, Oct 30 2025) [💀🔧⚠️→✅]
...
**INTENTIONAL SPEC DEVIATION**: create command fails if file exists (Session d53d3ec2, Oct 30 2025) [⚠️🔒💡]
...
MCP server supports dual tool exposure modes: unified tool with command parameter (default) vs separate tools per command [🏗️💡]
...
Zod .refine() creates ZodEffects wrapper which breaks discriminated unions and .shape property access - for discriminated unions, move validation logic into the handler function instead of schema refinement [⚠️🔧]
...
MCP-Debug is useful for quick iteration BUT always verify with actual Claude Code MCP client before marking complete [⚠️🧪]
...
Dual schema locations for parameter changes: When adding optional params, update BOTH MemoryCommandSchema (21-64) AND individual command schemas (76-119) in src/server/mcp-server.ts [🏗️💡]
